...
app.get('/', function(req, res, next) {
	req.getConnection(function(error, conn) {
		conn.query('SELECT * FROM instituicoes ORDER BY id DESC',function(err, rows, fields) {
			//redirecionamento
			if (err) {
				req.flash('error', err)
...
app.post('/add', function(req, res, next){	
    ...
    if( !errors ) {   
		...
		req.getConnection(function(error, conn) {
			conn.query('INSERT INTO instituicoes SET ?', user, function(err, result) {
				...
				if (err) {
					...
					res.render('user/add', {
						...
						bairro: user.bairro,
						cidade: user.cidade,
						data_de_inauguracao: user.data_de_inauguracao,
						timestamp: user.timestamp					
					})
				} else {				
					req.flash('Cadastro adicionado com sucesso!')
					
					// Redirecionamento para adicionar um novo local
					res.render('user/add', {
						title: 'Adicionar novo local',
						nome: '',
						rua: '',
